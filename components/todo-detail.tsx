...
export default function TodoDetail({
  ...
  const updateDueDate = async (dueDate: string) => {
    ...
    try {
      ...
      setTodo((prev) =>
        prev ? { ...prev, due_date: dueDate || undefined } : null
      );
    } catch (err: any) {
      ...
      <div className="max-w-4xl mx-auto p-6">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            The todo you're looking for doesn't exist or you don't have
      ...
      <div className="border rounded-lg shadow-lg p-6 mb-6">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center text-white font-semibold">
            {isOwner
              ? "You"
              : todo.todo_shares[0].owner_email.charAt(0).toUpperCase()}
          </div>
          <div>
            ...
            <p className="text-sm ">
              Owner • Created{" "}
              {todo.created_at ? formatDateTime(todo.created_at) : "Unknown"}
            </p>
          </div>
        </div>
      </div>
          ...
          {todoShares.length === 0 ? (
            ...
            <div className="space-y-3">
              {todoShares.map((share, index) => (
                <div
                  key={index}
                  className="border flex items-center justify-between p-3 flex-col sm:flex-row rounded gap-2 sm:gap-1"
                >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateSharePermission(
                          ...
                          e.target.value as "read" | "read_write"
                        )
                      }
